...
st.sidebar.caption("註：大額存款（如五百萬以上）利率通常較一般存款低")
...
# 模擬一個季度 (3個月) 的收益與風險表現
# 方案 A: 全押 1M Rolling (高流動性)
# 方案 B: 全押 3M 定存 (中流動性)
# 方案 C: 全押 6M 定存 (低流動性)
# 方案 D: 法人階梯拆單 (將定存資產均分 1M, 2M, 3M 展期，使其每月均有 1/3 到期)

# 基礎正常利息收益 (3個月)
interest_demand = demand_amount * (
    (1 + r_d) ** 3 - 1
)  # 活存部分的收益（若未發生出金）
...
# 方案 C (6M) 正常利息 (未滿6M解約，通常依實際存期打八折，此處假設持有3個月按3個月利率打八折)
...
) + interest_demand  # 複合估算

# --- 風險與流動性懲罰計算 (Liquidty Penalty) ---
# 當發生 liquidity_gap 時，定存必須被迫「中途解約」
# 台灣銀行中途解約規定：未滿 1 個月不計息；滿 1 個月未滿 3 個月按 1M 利率打 8 折。
penalty_A = 0
penalty_B = 0
penalty_C = 0
penalty_D = 0
...
if liquidity_gap > 0:
    # 方案 A 全是 1M，每個月都到期，通常只需承受當月未滿一個月的解約損失，風險相對低
    penalty_A = liquidity_gap * r1 * 0.5  # 期望值折損
    # 方案 B 全是 3M，一旦中途解約，大筆資金利率直接退回 1M 再打 8 折
    penalty_B = liquidity_gap * (rate_3m / 100 - (rate_1m / 100) * 0.8) * (2 / 12)
    # 方案 C 全是 6M，鎖死最嚴重，提前解約懲罰最高
    penalty_C = liquidity_gap * (rate_6m / 100 - (rate_1m / 100) * 0.7) * (3 / 12)
    # 方案 D 階梯拆單：因為每個月都有 1/3 到期，只有剩餘缺口需要被迫解約
...
p2 = ax.barh(
    ...
    label="流動性懲罰 (Liquidity Penalty)",
    color="#E63946",
)

ax.set_yticks(y_pos)
ax.set_yticklabels(df_fcm["FCM 存款配置策略"])
...
if liquidity_gap > 0:
    ...
    st.success(
        "✅ **安全**：目前活期留存金相當充足，足以應付突發出金。可以安心進行長天期大額定存佈局以套取利差。"
    )

with st.expander("📌 期貨商保證金管理的財務操作精髓"):
    st.markdown(
        """
    * **大額存款壁壘**：金額一旦超過 500 萬（甚至法人級的 5,000 萬），銀行端給予的利率會呈現雪崩式下滑。本模擬器已自動帶入台銀 2026 大額存款利率（約 $0.71\%\sim0.72\%$），這與一般散戶看到的 $1.4\%$ 截然不同。因此期貨商財務處更看重**資金流動效率**與**分散在複數銀行（避免單一銀行集中度風險）**。
